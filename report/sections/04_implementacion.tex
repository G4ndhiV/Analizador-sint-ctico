...
\subsection{Fragmento destacado: integración lex $\leftrightarrow$ yacc}

El mecanismo de indentación Python requiere que el scanner inyecte tokens
\texttt{INDENT} y \texttt{DEDENT} que el parser consume como cualquier otro
terminal. La función \texttt{bol\_dedent\_token()} en el scanner detecta
cuando un identificador aparece en columna~0 y emite el \texttt{DEDENT}
primero, difiriendo el token real:

\lstinputlisting[language=C, firstline=364, lastline=420, caption={Detección de DEDENT en columna 0 y diferimiento de token (\texttt{lexer/lexico\_equipo\_11\_scanner.l}).}]{../lexer/lexico_equipo_11_scanner.l}

El parser consume estos tokens sintéticos en la regla \texttt{suite}:

\lstinputlisting[language=C, firstline=295, lastline=325, caption={Regla \texttt{suite} en \texttt{parser/parser.y}: uso de INDENT/DEDENT.}]{../parser/parser.y}
